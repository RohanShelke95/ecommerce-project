...
export default function AdminNavbar({handleSideBarViewInMobile}) {
  ...
  const renderMenu = (
    ...
    >
      <MenuItem onClick={()=>{handleMenuClose(); navigate("/account/profile")}}>Profile</MenuItem>
      <MenuItem onClick={()=>{handleMenuClose(); localStorage.removeItem("jwt"); window.location.href="/"}}>Logout</MenuItem>
  ...
  const renderMobileMenu = (
    ...
    >
      <MenuItem onClick={() => alert("No new messages")}>
      ...
      <MenuItem onClick={() => alert("No new notifications")}>
        <IconButton
          size="large"
      ...
      <AppBar position="fixed" sx={{ zIndex: (theme) => theme.zIndex.drawer + isLargeScreen, backgroundColor: 'rgb(0, 0, 22)' }}>
        <Toolbar>
          ...
          <Box sx={{ display: { xs: 'none', md: 'flex' } }}>
            <IconButton size="large" aria-label="show 4 new mails" color="inherit" onClick={() => alert("No new messages")}>
              <Badge badgeContent={0} color="error">
                <MailIcon />
              </Badge>
            </IconButton>
